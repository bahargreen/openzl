...
#
#         # stats["decomposed openzl_sddl_groups col-order comp time s"] = float(t_sddl_F)
#         # stats["decomposed openzl_sddl_groups row-order comp time s"] = float(t_sddl_C)
#
...
#
#         if sddlh.SDDL_MBPS_DENOM == "orig":
#             stats["decomposed openzl_sddl_groups col-order comp MB/s"] = sddlh._mbps(len_bytes, t_sddl_F)
#            # stats["decomposed openzl_sddl_groups row-order comp MB/s"] = sddlh._mbps(len_bytes, t_sddl_C)
...
#         else:
#             stats["decomposed openzl_sddl_groups col-order comp MB/s"] = sddlh._mbps(nproc_F, t_sddl_F)
#            # stats["decomposed openzl_sddl_groups row-order comp MB/s"] = sddlh._mbps(nproc_C, t_sddl_C)
...
Main benchmark script for:
- standard SDDL
- grouped-record SDDL for decomposed byte groups

Notes:
- SDDL decodes the record stream defined by the schema.
- For grouped/decomposed SDDL, full restore still includes:
    SDDL decode + inverse byte-group reconstruction + float view
- This script reports both:
    1) core decomp time / MB/s
    2) full restore time / MB/s
