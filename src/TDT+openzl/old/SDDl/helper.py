...
def _is_no_reorder_decomp(decomp, m: int) -> bool:
    ...
    Expects zero-based indices, e.g. [[0,1],[2],[3]]
    """
    flat = []
    for group in decomp:
...
def _build_grouped_packed_bytes_col_fast(
    ...
    """
    Build grouped packed bytes in column-major serialization.
...
# ============================================================
# Cached public builders
# ============================================================

@lru_cache(maxsize=None)
def get_standard_sddl_tool(bytes_per_value: int):
    return _OpenZLSDDLStandard(bytes_per_value)


@lru_cache(maxsize=None)
def get_grouped_sddl_tool(group_lens: tuple[int, ...]):
    return _OpenZLSDDLGroups(group_lens)
